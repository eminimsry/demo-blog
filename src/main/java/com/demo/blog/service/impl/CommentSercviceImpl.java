...
public class CommentSercviceImpl implements CommentService {
    ...
    @Override
    public List<Comment> listCommentByBid(Long bolgId) {
        Sort sort = new Sort(Sort.Direction.ASC,"createTime");//指定排序方式(按时间排序)
        List<Comment> comments = commentMapper.findByBlogIdAndParentCommentNull(bolgId,sort);
    ...
    public Comment saveComment(Comment comment) {
        ...
        if (pid != -1){
            comment.setParentComment(commentMapper.getOne(pid));
        }else {
